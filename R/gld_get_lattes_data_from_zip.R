...
#' }
function (zip.files, field.qualis = NULL) 
{
  ...
  suppressWarnings({
    tpesq <- do.call(args = lapply(my.l, function(x) x$tpesq), 
                     what = dplyr::bind_rows)
    tpublic.published <- do.call(args = lapply(my.l, function(x) x$tpublic.published), 
                                 what = dplyr::bind_rows)
    tpublic.accepted <- do.call(args = lapply(my.l, function(x) x$tpublic.accepted), 
                                what = dplyr::bind_rows)
    tsupervisions <- do.call(args = lapply(my.l, function(x) x$tsupervisions), 
                             what = dplyr::bind_rows)
    tbooks <- do.call(args = lapply(my.l, function(x) x$tbooks), 
                      what = dplyr::bind_rows)
    tconferences <- do.call(args = lapply(my.l, function(x) x$tconferences), 
                            what = dplyr::bind_rows)
  })
  ...
  idx <- unlist(sapply(stringr::str_replace_all(tpublic.published$ISSN,
                                                "-", ""), function(issn.in, df.sjr) {
                                                  ...
                                                  if (stringr::str_trim(issn.in) == "") 
                                                    return(NA)
                                                  if (length(temp.idx) == 0) {
                                                    temp.idx <- NA
                                                  }
                                                  return(temp.idx[1])
                                                }, df.sjr = df.sjr, USE.NAMES = F))
  if(sum(is.na(idx)) > 0) {
    idx <- idx[!is.na(idx)]
    if(length(idx) != 0) {
      ...
      tpublic.published$SJR <- 0
      tpublic.published$H.SJR <- 0
    }
  }
  ...
  suppressWarnings({
    ...
    tpesq$phd.start.year <- ifelse(is.null(tpesq$phd.start.year), 0, as.numeric(tpesq$phd.start.year))
    tpesq$phd.end.year <- ifelse(is.null(tpesq$phd.end.year), 0, as.numeric(tpesq$phd.end.year))
  ...
  tpublic.published <- as.data.frame(lapply(tpublic.published, 
                                            my.enc.fct), stringsAsFactors = F)
  tpublic.accepted <- as.data.frame(lapply(tpublic.accepted, 
                                           my.enc.fct), stringsAsFactors = F)
  tsupervisions <- as.data.frame(lapply(tsupervisions, my.enc.fct), 
                                 stringsAsFactors = F)
  tbooks <- as.data.frame(lapply(tbooks, my.enc.fct), stringsAsFactors = F)
  tconferences <- as.data.frame(lapply(tconferences, my.enc.fct), 
                                stringsAsFactors = F)
  l.out <- list(tpesq = tpesq, tpublic.published = tpublic.published, 
                tpublic.accepted = tpublic.accepted, tsupervisions = tsupervisions, 
                tbooks = tbooks, tconferences = tconferences)
  return(l.out)
}
